      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full">
            ...
            <div>
              ...
              <div className="product-detail-reveal border border-stone-900 rounded-xl bg-stone-900/10 p-6 mb-8">
                ...
                <dl className="grid grid-cols-2 gap-y-4 gap-x-6 text-xs">
                  <div>
                    <dt className="text-stone-500 font-medium">Country of Origin</dt>
                    <dd className="text-white mt-0.5">{product.spec.origin}</dd>
                  </div>
                  <div>
                    <dt className="text-stone-500 font-medium">Finishes Available</dt>
                    <dd className="text-white mt-0.5">{product.spec.finish.join(', ')}</dd>
                  </div>
                  <div>
                    <dt className="text-stone-500 font-medium">Standard Slab Sizes</dt>
                    <dd className="text-white mt-0.5">{product.spec.sizes.join(', ')}</dd>
                  </div>
                  <div>
                    <dt className="text-stone-500 font-medium">Best Applications</dt>
                    <dd className="text-white mt-0.5">{product.spec.application.join(', ')}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div className="product-detail-reveal mb-8">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {product.features.map((feat) => (
                    <li key={feat} className="flex items-start gap-2 text-xs text-stone-400">
        ...
        {relatedProducts.length > 0 && (
          <section className="border-t border-stone-900 pt-16">
            <h2 className="font-serif text-2xl text-white uppercase tracking-wider mb-8">
              Related <span className="italic text-gold-foil">Products</span>
            </h2>
                ...
                <div key={p.slug} className="group flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <Link href={`/products/${p.slug}`}>
                      <h4 className="font-serif text-base text-white uppercase font-medium hover:text-gold-400 transition-colors mt-1">
                        {p.name}
                      </h4>
                    </Link>
                    <div className="flex justify-between items-center text-xs mt-2">
                      ...
                      <Link 
                        href={`/products/${p.slug}`}
                        className="text-[10px] text-gold-400 uppercase font-bold tracking-widest inline-flex items-center gap-1 hover:text-white transition-colors"
                      >
                        <span>View</span>
