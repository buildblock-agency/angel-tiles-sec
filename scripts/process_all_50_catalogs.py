...
for pdf_name in sorted(pdf_files):
    ...
    # Select representative 1-3 pages per catalog
    if page_count == 1:
        pages_to_pick = [0]
    elif page_count == 2:
        pages_to_pick = [0, 1]
    else:
        pages_to_pick = [1, min(page_count - 1, 3)]

    for p_idx in pages_to_pick:
...
print(f"\nSuccessfully generated {len(manifest)} WebP products from ALL PDF catalogs!")
